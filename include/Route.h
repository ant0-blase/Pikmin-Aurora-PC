...
public:
	...
	{
		// FourCC values are stored in canonical big-endian order (e.g. 'test'
		// == 0x74657374), regardless of the host CPU's byte order. The original
		// GameCube code could alias the integer directly because PowerPC is
		// big-endian; doing that on x86_64 reverses the textual ID.
		mIntID       = id;
		mStringID[0] = static_cast<char>((id >> 24) & 0xFF);
		mStringID[1] = static_cast<char>((id >> 16) & 0xFF);
		mStringID[2] = static_cast<char>((id >> 8) & 0xFF);
		mStringID[3] = static_cast<char>(id & 0xFF);
		mStringID[4] = '\0';
	}
	...
	{
		// Keep the numeric value compatible with multi-character constants used
		// throughout the game (for example RouteMgr queries using 'test').
