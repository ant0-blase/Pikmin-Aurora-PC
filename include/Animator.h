...
struct FrameCacher : public CacheInfo {
	// This mirrors the prefix of AnimCacheInfo. The original GameCube layout
	// used 32-bit pointers and a u32 tail for both the pointer table and matrix
	// storage. On a 64-bit host that truncates Matrix4f* entries and places the
	// matrix block halfway through the pointer table.
	CacheInfo** mInfo;
	u8 _10[4];
	Matrix4f* mBoneMatricesStart;
