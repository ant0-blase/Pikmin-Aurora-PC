...
typedef intptr_t MenuData;
#else
typedef int MenuData;
#endif
...
public:
	...
	class MenuItem {
	public:
		MenuItem(int itemType, MenuData data, char* label, IDelegate1<Menu&>* confirmCallback);
		...
		MenuData mData;           ///< _1C, data payload; pointer-width on native PC, int on GameCube.
	...
	void resetOptions();
	void addOption(MenuData data, char* label, IDelegate1<Menu&>* confirmCallback, bool isEnabled = true);
	void addMenu(Menu* subMenu, MenuData data, char* label);
