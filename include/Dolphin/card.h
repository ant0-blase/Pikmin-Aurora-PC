...
// Aurora's host CARD API needs the disc game and maker codes during
// initialization.  The original GameCube SDK entry point took no arguments.
...
#if defined(TARGET_PC)
// Aurora host API: delete by canonical filename. Useful with the GCI-folder backend,
// where directory indices are not guaranteed to behave like the physical SDK table.
s32 CARDDelete(s32 channel, const char* fileName);
#endif
