...
public:
	...
	// The original game declares this const and mutates it once through const_cast.
	// Modern ELF toolchains place that definition in read-only memory, so the
	// first Graphics construction would SIGSEGV. Keep the matching declaration
	// for GameCube, but give the native port writable storage.
