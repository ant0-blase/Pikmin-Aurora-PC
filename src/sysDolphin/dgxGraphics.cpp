...
#if defined(TARGET_PC)
// aurora_end_frame() must only be paired with a successful begin. During a
// Wayland/X11 minimize, focus transition or fullscreen swapchain rebuild,
// aurora_begin_frame() can legitimately return false for a host frame.
static bool sHostAuroraFrameActive = false;
#endif
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] DGXGraphics ctor body begin this=%p\n", this);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] DGX buffers fifo=%p temp=%p dl=%p\n", mDefaultFifoBuffer, mTempFifoBuffer, mDefaultDLBuffer);
#endif
	mGpFifo            = GXInit(mDefaultFifoBuffer, kDefaultFifoSize);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GXInit done fifoObj=%p\n", mGpFifo);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GX display-copy setup done %dx%d\n", mScreenWidth, mScreenHeight);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GX pixel format ready\n");
#endif
	VIInit();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] VIInit done, entering videoReset\n");
#endif
	videoReset();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] videoReset done\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] preparing display buffer size=%u activeHeap=%d\n", sFrameSize, gsys->mActiveHeapIdx);
#endif
	int backup = gsys->getHeap(gsys->mActiveHeapIdx)->mAllocType;
	gsys->getHeap(gsys->mActiveHeapIdx)->setAllocType(AYU_STACK_GROW_UP);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] heap switched to grow-up, allocating display buffer\n");
#endif

	mDisplayBuffer = new (0x20) u8[sFrameSize];
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] display buffer=%p size=%u\n", mDisplayBuffer, sFrameSize);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] display buffer primed\n");
#endif
#if defined(VERSION_PIKIDEMO) || defined(VERSION_GPIJ01)
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] initial VI retrace done\n");
#endif
	setupRender();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] setupRender done\n");
#endif
	gfx                   = this;
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] retrace callback/message queue ready\n");
#endif
...
#endif
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] DGXGraphics ctor complete\n");
#endif
}
...
void DGXGraphics::beginRender()
{
	...
	gsys->mIsRendering = 1;
#if defined(TARGET_PC)
	sHostAuroraFrameActive = aurora_begin_frame();
#endif
	GXInvalidateTexAll();
...
void DGXGraphics::doneRender()
{
	GXFlush();
#if defined(TARGET_PC)
	// Aurora owns the host frame lifecycle. Never submit an end-frame after a
	// failed begin-frame (common for a minimized/unpresentable window), or Dawn
	// can race a surface/device teardown.
	if (sHostAuroraFrameActive) {
		aurora_end_frame();
	}
	sHostAuroraFrameActive = false;
#else
	GXDrawDone();
#endif
	gsys->mIsRendering = 0;
...
{
#if defined(TARGET_PC)
	// Do not block waiting for an asynchronous callback thread on the host.
	// Pace any extra 30 Hz clamp retraces synchronously; VIWaitForRetrace() also
	// pumps the pending retrace callback on this same main thread.
 ...
 * @todo: Documentation
 */
void DGXGraphics::retraceProc(u32 retrace)
{
#if defined(TARGET_PC)
	static u32 hostRetraceTraceCount = 0;
	const bool traceRetrace = hostRetraceTraceCount < 12;

	// The original GameCube build keeps a second global copy of the active
	// DGXGraphics pointer in DGXGraphics::gfx.  On the 64-bit native port that
	// legacy BSS slot is not authoritative (and can be clobbered by remaining
	// 32-bit-layout code).  System::mDGXGfx is the owning pointer used by the
	// entire host render path, so resolve the callback target from it instead.
	DGXGraphics* activeGfx = gsys ? static_cast<DGXGraphics*>(gsys->mDGXGfx) : nullptr;
	if (traceRetrace) {
		OSReport("[pikmin::vi] retraceProc begin retrace=%u gsys=%p activeGfx=%p legacyGfx=%p loading=%d\n",
		         retrace, gsys, activeGfx, gfx, gsys ? gsys->mIsLoadingActive : -1);
	}
	if (!gsys || !activeGfx) {
		OSPanic(__FILE__, __LINE__, "host retrace callback has no active graphics object");
	}
	if (gfx != activeGfx && traceRetrace) {
		OSReport("[pikmin::vi] legacy DGXGraphics::gfx is stale/corrupt (%p); using System::mDGXGfx=%p\n",
		         gfx, activeGfx);
	}
#else
	DGXGraphics* activeGfx = gfx;
#endif
	...
	if (gsys->mIsLoadingActive) {
		gsys->nudgeLoading();
#if defined(TARGET_PC)
		if (traceRetrace) OSReport("[pikmin::vi] retraceProc loading path complete\n");
		++hostRetraceTraceCount;
#endif
		return;
	}

	gsys->mRetraceCount++;
	if (activeGfx->mPostRetraceWaitCount != 0 && --activeGfx->mPostRetraceWaitCount == 0) {
		OSSendMessage(&activeGfx->mPostRetraceMsgQueue, (OSMessage)'DONE', 0);
	}
#if defined(TARGET_PC)
	if (traceRetrace) OSReport("[pikmin::vi] retraceProc complete systemRetraces=%u wait=%d\n", gsys->mRetraceCount, activeGfx->mPostRetraceWaitCount);
	++hostRetraceTraceCount;
#endif
}
...
{
#if defined(TARGET_PC)
	// Aurora scales and applies the logical scissor itself. The original SDK's
	// scissor-box offset register has no host equivalent at this time.
	(void)bounds;
#else
...
void DGXGraphics::initMesh(Shape* model)
{
	...
	if (model->mVertexList) {
		...
		GXSETARRAY(GX_VA_POS, model->mVertexList, model->mVertexCount * sizeof(Vector3f), sizeof(Vector3f), true);
	}
	...
	if (model->mNormalList) {
		...
		GXSETARRAY(GX_VA_NRM, model->mNormalList, model->mNormalCount * sizeof(Vector3f), sizeof(Vector3f), true);
	}
	...
	for (i = 0; i < model->mTotalActiveTexCoords; i++) {
		if (model->mTexCoordList[i]) {
			...
			GXSETARRAY((GXAttr)(GX_VA_TEX0 + i), model->mTexCoordList[i], model->mTexCoordCounts[i] * sizeof(Vector2f), sizeof(Vector2f), true);
		}
	}
	...
	if (model->mVtxColorList) {
		...
		GXSETARRAY(GX_VA_CLR0, model->mVtxColorList, model->mVtxColorCount * sizeof(Colour), sizeof(Colour), true);
	}
...
void DGXGraphics::setupVtxDesc(Shape* model, Material* mat, Mesh* mesh)
{
	...
	if (mCustomScale) {
		if (!sendNbtIndx) {
			GXSETARRAY(GX_VA_NBT, model->mNBTList, model->mNBTCount * sizeof(NBT), sizeof(NBT), true);
...
#if defined(TARGET_PC)
typedef u8 DGXRawByte;
#else
typedef char DGXRawByte;
#endif

static DGXRawByte sAsciiTable[] = {
