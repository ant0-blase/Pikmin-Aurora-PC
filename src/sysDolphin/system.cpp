...
#include <SDL3/SDL.h>
extern "C" void pikmin_host_audio_set_paused(BOOL paused);
#endif
...
#if defined(TARGET_PC)
// P2D historically creates a second Font wrapper for every .bfn reference.
// The native port already builds the canonical bigFont once during System
// initialisation, on a lifetime-stable heap.  Expose it to the P2D host shim
// so UI text does not walk/extend the transient GfxobjInfo cache during a
// section reset.
Font* PikminHostGetBigFont()
{
	return bigFont;
}
#endif
...
#if defined(TARGET_PC)
static CacheTexture* sPendingTextureCopy = nullptr;
#endif
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::run begin app=%p\n", app);
	u32 hostFrameIndex = 0;
#endif
	GXInvalidateTexAll();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::run GXInvalidateTexAll done\n");
	bool firstHostFrame = true;
#endif

#if defined(TARGET_PC)
	bool hostPaused = false;
	int hostSurfaceSettleFrames = 0;
#endif
	while (true) {
#if defined(TARGET_PC)
		const bool traceHostFrame = hostFrameIndex < 4;
		if (traceHostFrame) OSReport("[pikmin::frame] %u begin aurora_update\n", hostFrameIndex);
		const AuroraEvent* event = aurora_update();
		bool exitRequested = false;
		bool fullscreenChanged = false;

		// aurora_update() returns a zero-terminated *array* of events. The old
		// native loop only looked at element zero, so focus/minimize/resume and
		// resize events were silently lost. That let Pikmin keep rendering while
		// Aurora was rebuilding or releasing its Vulkan surface.
		for (; event && event->type != AURORA_NONE; ++event) {
			...
			case AURORA_PAUSED:
				hostPaused = true;
				pikmin_host_audio_set_paused(TRUE);
				OSReport("[pikmin::window] paused (focus/minimize)\n");
				break;
			case AURORA_UNPAUSED:
				hostPaused = false;
				hostSurfaceSettleFrames = std::max(hostSurfaceSettleFrames, 2);
				pikmin_host_audio_set_paused(FALSE);
				OSReport("[pikmin::window] resumed; waiting for surface\n");
				break;
			case AURORA_WINDOW_RESIZED:
			case AURORA_DISPLAY_SCALE_CHANGED:
				hostSurfaceSettleFrames = std::max(hostSurfaceSettleFrames, 1);
				break;
			case AURORA_SDL_EVENT:
				if (event->sdl.type == SDL_EVENT_KEY_DOWN && !event->sdl.key.repeat) {
					const bool altEnter = event->sdl.key.key == SDLK_RETURN && (event->sdl.key.mod & SDL_KMOD_ALT);
					if (event->sdl.key.key == SDLK_F11 || altEnter) {
						const bool fullscreen = !VIGetWindowFullscreen();
						VISetWindowFullscreen(fullscreen);
						hostSurfaceSettleFrames = std::max(hostSurfaceSettleFrames, 2);
						fullscreenChanged = true;
						OSReport("[pikmin::window] fullscreen=%d (%s)\n", fullscreen ? 1 : 0,
						         event->sdl.key.key == SDLK_F11 ? "F11" : "Alt+Enter");
					}
				}
				break;
			default:
		...
		if (traceHostFrame) OSReport("[pikmin::frame] %u aurora_update done; paused=%d settle=%d\n",
		                              hostFrameIndex, hostPaused ? 1 : 0, hostSurfaceSettleFrames);
		if (hostPaused || fullscreenChanged || hostSurfaceSettleFrames > 0) {
			if (!hostPaused && hostSurfaceSettleFrames > 0) --hostSurfaceSettleFrames;
			continue;
		}

		// Keep VI/retrace callbacks on the same thread as the game and Aurora.
		// The host VI clock only advances a counter in the background.
		VIHostPumpCallbacks();
		if (traceHostFrame) OSReport("[pikmin::frame] %u VI pump done; Jac_Gsync\n", hostFrameIndex);
#endif
		Jac_Gsync();
#if defined(TARGET_PC)
		if (traceHostFrame) OSReport("[pikmin::frame] %u Jac_Gsync done; CARDProbe\n", hostFrameIndex);
#endif
		CARDProbe(0);
		CARDProbe(1);
#if defined(TARGET_PC)
		if (traceHostFrame) OSReport("[pikmin::frame] %u CARDProbe done; controller update\n", hostFrameIndex);
#endif
		mControllerMgr.update();
#if defined(TARGET_PC)
		if (traceHostFrame) OSReport("[pikmin::frame] %u controller update done; clock/threads\n", hostFrameIndex);
#endif
		updateSysClock();
		OSCheckActiveThreads();
#if defined(TARGET_PC)
		if (traceHostFrame) OSReport("[pikmin::frame] %u clock/threads done; app->idle\n", hostFrameIndex);
#endif
		app->idle();
#if defined(TARGET_PC)
		if (traceHostFrame) OSReport("[pikmin::frame] %u app->idle returned\n", hostFrameIndex);
#endif
#if defined(TARGET_PC)
		if (firstHostFrame) {
			OSReport("[pikmin::boot] first host frame complete\n");
			firstHostFrame = false;
		}
		if (traceHostFrame) OSReport("[pikmin::frame] %u complete\n", hostFrameIndex);
		++hostFrameIndex;
#endif
	}
...
void System::parseArchiveDirectory(immut char* arcPath, immut char* dirPath)
{
	...
	while (pend != 0) {
		...
		gsys->copyRamToCache((uptr)DVDStream::readBuffer, size, a + pos);
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::hardReset begin heap=%d\n", mActiveHeapIdx);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::hardReset allocating TextureCacher\n");
#endif
	mCacher  = new TextureCacher(0x96000);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::hardReset TextureCacher=%p\n", mCacher);
#endif
#if defined(TARGET_PC)
	// The original 0x20000-byte language heap was sized for 32-bit PowerPC
	// objects. Native x86_64 objects contain 64-bit pointers and the texture
	// metadata loaded from screen.bun is substantially larger, so give the
	// host port enough headroom while keeping the original console size.
	int size = 0x80000;
#else
	int size = 0x20000;
#endif
	void* languageHeap = alloc(size);
	gsys->mHeaps[SYSHEAP_Lang].init("language", AYU_STACK_GROW_UP, languageHeap, size);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::hardReset language heap=%p size=%d\n", languageHeap, size);
	OSReport("[pikmin::boot] System::hardReset entering preloadLanguage\n");
#endif
	preloadLanguage();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::hardReset preloadLanguage complete\n");
#endif
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::Initialise begin\n");
#endif
	OSInit();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] OSInit done\n");
	// Aurora's CARDInit host ABI takes the GameCube game/maker IDs.  Calling
	// the original zero-argument SDK signature leaves both parameters as
	// garbage registers and crashes immediately after Aurora's CARD init log.
	...
	CARDInit("GPIE", "01");
#endif
	OSReport("[pikmin::boot] CARDInit done\n");
#else
	CARDInit();
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] arena lo=%p hi=%p\n", lo, hi);
#endif
	mHeapStart = OSRoundUp32B(OSInitAlloc(lo, hi, 1));
	hi         = (void*)OSRoundDown32B(hi);
	mHeapEnd   = (u32)((uptr)hi - mHeapStart);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] heap start=%p size=%u bytes\n", (void*)mHeapStart, mHeapEnd);
#endif
#if defined(VERSION_GPIP01)
	...
	setHeap(SYSHEAP_Sys);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] Pikmin system heap active\n");
#endif
	...
	onceInit();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] matrix pool ready\n");
#endif

	mDGXGfx          = new DGXGraphics(false);
	mGraphics        = mDGXGfx;
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] DGXGraphics ready\n");
#endif
	mIsRendering     = 0;
	mIsLoadingActive = 0;
	...
	initBigFont();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] built-in font ready\n");
#endif
	startDvdThread();
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] bigFont.bti loaded\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] audio compatibility layer ready\n");
#endif
	...
	mConsFont = cons;
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] console font ready\n");
#endif

	endLoading();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] System::Initialise complete\n");
#endif
}
...
#if defined(TARGET_PC)
		if (reinterpret_cast<uptr>(msg) == static_cast<uptr>('QUIT')) {
#else
		if ((u32)msg == 'QUIT') {
#endif
...
#if defined(TARGET_PC)
		// Aurora/Dawn owns a single host GPU context. The original GameCube loading
		// thread used GXSetCurrentGXThread() to transfer GX ownership, but Aurora
		// has no equivalent. Rendering from a std::thread here and then returning
		// to aurora_update() on the main thread races the Vulkan backend. Keep the
		// game's loading state but defer all GX/Aurora rendering to the main thread.
		if (idler) {
			idler->init();
		}
		mIsLoadingActive = 1;
		OSReport("[pikmin::loading] host loading renderer deferred to main thread\n");
#else
...
void System::endLoading()
{
	...
	if (mIsLoadingActive) {
#if defined(TARGET_PC)
		// No worker exists on native PC; startLoading only tracks the original
		// loading state so game-flow logic can remain unchanged.
		OSReport("[pikmin::loading] host loading state complete\n");
#else
 ...
 */
void doneDMA(uptr cache)
{
	// free the cache
	SystemCache* sysCache = reinterpret_cast<SystemCache*>(cache);
	sysCache->remove();
 ...
 */
u32 System::copyRamToCache(uptr mainMemAddr, u32 size, u32 aramCacheAddr)
{
	...
	ARQPostRequest(cache, 0, ARQ_TYPE_MRAM_TO_ARAM, ARQ_PRIORITY_HIGH, mainMemAddr, adjustedCacheAddr, size, doneDMA);
 ...
 * @param size Size of data to copy (bytes).
 */
void System::copyCacheToRam(uptr mainMemAddr, u32 aramCacheAddr, u32 size)
{
	// wait until any active DMA requests are done
	copyWaitUntilDone();
	...
	// send request to the aram queue (high priority)
	ARQPostRequest(cache, 0, ARQ_TYPE_ARAM_TO_MRAM, ARQ_PRIORITY_HIGH, aramCacheAddr, mainMemAddr, size, doneDMA);
}
 ...
 */
void freeBuffer(uptr cache)
{
	#if defined(TARGET_PC)
	CacheTexture* texCache = sPendingTextureCopy;
	sPendingTextureCopy = nullptr;
#else
...
void System::copyCacheToTexture(CacheTexture* tex)
{
	...
	uptr mainMemAddr = (uptr)tex->mTexImage->mTextureData;
...
#if defined(TARGET_PC)
	sPendingTextureCopy = tex;
#endif
	DCInvalidateRange((void*)mainMemAddr, size);
	ARQPostRequest(cache, 0, ARQ_TYPE_ARAM_TO_MRAM, ARQ_PRIORITY_HIGH, aramAddr, mainMemAddr, size, freeBuffer);
...
{
#if defined(TARGET_PC)
	// Aurora reads from an already-mounted ISO image. There is no physical DVD
	// drive state to poll, and waking the original drive-monitor worker from the
	// host retrace callback only introduces cross-thread races.
	return;
#else
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::dvd] host drive-monitor worker disabled (Aurora ISO backend)\n");
#else
