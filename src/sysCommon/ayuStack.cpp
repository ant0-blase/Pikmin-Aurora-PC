...
void AyuStack::create(immut char* name, int allocFlags, void* stackBase, int stackSizeBytes, bool enableOverflowGuard)
{
	...
	mIsActive          = true;
	mName              = name;
	mInitialStackTop   = (uptr)stackBase;
...
void AyuStack::checkStack()
{
	if ((mAllocType & AYU_STACK_GROW_UP)) {
		uptr newTop = mStackTop - *(u32*)(mStackTop - 8);
...
AyuCache::AyuCache(u32 cacheSize)
{
	...
	init((uptr)bufAddr, (uptr)bufAddr + alignedSize);
}
 ...
 */
void AyuCache::init(uptr bufferStart, uptr bufferEnd)
{
	...
	// Cache block arithmetic below advances MemHead pointers, so one allocation
	// unit must be sizeof(MemHead). That was 16 bytes on GameCube but is 32
	// bytes on x86-64 because MemHead contains native pointers.
	const uptr unitSize = sizeof(MemHead);
	head->mTagAndSize = ((bufferEnd - bufferStart) / unitSize) - 0x1000001;
...
{
	const u32 unitSize = sizeof(MemHead);
	u32 a          = (sizeBytes + unitSize - 1) / unitSize;
