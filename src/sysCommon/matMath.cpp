...
{
#if defined(TARGET_PC)
	// Dolphin MTXConcat operates on affine 3x4 matrices.  A number of original
	// Matrix4f producers only initialise those twelve values, so a native 4x4
	// multiply would read an undefined fourth row and subtly change the ABI.
	// Reproduce MTXConcat semantics explicitly and canonicalise the affine row.
	Matrix4f tmp;
	Matrix4f* dst = (&outMtx == this || &outMtx == &mtxB) ? &tmp : &outMtx;

	for (int r = 0; r < 3; r++) {
		for (int c = 0; c < 3; c++) {
			dst->mMtx[r][c] = mMtx[r][0] * mtxB.mMtx[0][c] + mMtx[r][1] * mtxB.mMtx[1][c]
			                  + mMtx[r][2] * mtxB.mMtx[2][c];
		}
		dst->mMtx[r][3] = mMtx[r][0] * mtxB.mMtx[0][3] + mMtx[r][1] * mtxB.mMtx[1][3]
		                  + mMtx[r][2] * mtxB.mMtx[2][3] + mMtx[r][3];
	}
	dst->mMtx[3][0] = 0.0f;
	dst->mMtx[3][1] = 0.0f;
	dst->mMtx[3][2] = 0.0f;
