...
void StdSystem::attachObjs()
{
	...
	u32 hostAttachCount = 0;
#endif
		for (GfxobjInfo* info = mGfxobjInfo.mNext; info != &mGfxobjInfo; info = info->mNext) {
			if (!info->mAttached) {
#if defined(TARGET_PC)
				if (hostAttachCount < 12 || (hostAttachCount & 0x3f) == 0) {
					OSReport("[pikmin::attach] begin #%u info=%p name=%s\n", hostAttachCount, info,
					         info->mString ? info->mString : "<null>");
				}
#endif
				info->attach();
				info->mAttached = true;
...
#if defined(TARGET_PC)
		OSReport("[pikmin::attach] complete count=%u\n", hostAttachCount);
#endif
 ...
 */
void StdSystem::invalidateObjs(uptr lowerBound, uptr upperBound)
{
	GfxobjInfo* next;
	for (GfxobjInfo* c = mGfxobjInfo.mNext; c != &mGfxobjInfo;) {
		next = c->mNext;

		uptr address = reinterpret_cast<uptr>(c);
...
void StdSystem::loadBundle(immut char* pPath, bool loadWithCache)
{
...
#if defined(TARGET_PC)
	OSReport("[pikmin::bundle] begin path=%s entries=%d cache=%d heap=%d free=%d\n",
	         pPath, fileCount, loadWithCache ? 1 : 0, gsys->mActiveHeapIdx,
	         gsys->getHeap(gsys->mActiveHeapIdx)->getFree());
#endif
	for (int i = 0; i < fileCount; i++) {
		...
		fs->readString(path);
#if defined(TARGET_PC)
		if ((i & 31) == 0 || gsys->getHeap(gsys->mActiveHeapIdx)->getFree() < 0x10000) {
			OSReport("[pikmin::bundle] entry=%d/%d type=%d size=%d path=%s free=%d\n",
			         i + 1, fileCount, type, size, path.mString ? path.mString : "<null>",
			         gsys->getHeap(gsys->mActiveHeapIdx)->getFree());
		}
#endif
		...
		case 1: // Texture
		{
			...
			if (loadWithCache) {
				...
				cacheTex->mAramAddress = copyRamToCache((uptr)texImg->mTextureData, texImg->mDataSize, 0);
				copyWaitUntilDone();
...
#if defined(TARGET_PC)
	OSReport("[pikmin::bundle] complete path=%s heap=%d free=%d\n", pPath, gsys->mActiveHeapIdx,
	         gsys->getHeap(gsys->mActiveHeapIdx)->getFree());
#endif
	fs->close();
}
...
void TextureCacher::cacheTexture(CacheTexture* tex)
{
	...
	while (true) {
		...
		if (largestBlockFree > texSize) {
			...
			tex->mTexImage->mTextureData = (void*)OSRoundDown32B((uptr)alloc + 0x33);
