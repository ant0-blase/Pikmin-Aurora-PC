...
#if defined(TARGET_PC)
extern "C" void pikmin_set_crash_context(const char* fmt, ...);
#endif
...
#if defined(WIN32) || defined(TARGET_PC)
...
{
	if (numTextures <= 0 || !info) {
		return;
	}

	// FrameCacher is intentionally layout-compatible with the fields read by
	// AnimData::makeAnimSRT after the block is cast to AnimCacheInfo. The tail
	// contains [numTextures Matrix4f* entries][numTextures Matrix4f entries].
	// Using native pointer widths here is mandatory on x86-64.
	const u32 headerSize = static_cast<u32>(sizeof(FrameCacher) - sizeof(uptr));
	const u32 listSize   = static_cast<u32>(sizeof(Matrix4f*) * numTextures);
	const u32 matrixSize = static_cast<u32>(sizeof(Matrix4f) * numTextures);
	const u32 texSize    = OSRoundUp32B(headerSize + listSize + matrixSize);

	while (true) {
		u32 freeSize = mCache->largestBlockFree();
		if (freeSize >= texSize) {
			FrameCacher* cacher = static_cast<FrameCacher*>(mCache->mallocL(texSize));
			if (!cacher) {
				return;
			}

			u8* storage                 = reinterpret_cast<u8*>(&cacher->mBoneStorage[0]);
			cacher->mBoneMtxList        = reinterpret_cast<Matrix4f**>(storage);
			cacher->mBoneMatricesStart  = reinterpret_cast<Matrix4f*>(storage + listSize);
			cacher->mInfo               = &info->mCachedMtxBlock;

			for (int i = 0; i < numTextures; i++) {
				cacher->mBoneMtxList[i] = nullptr;
			}

#if defined(TARGET_PC)
			static bool sReportedFrameCacheLayout = false;
			if (!sReportedFrameCacheLayout) {
				OSReport("[pikmin::anim-cache] native layout header=%u ptr=%zu matrix=%zu entries=%d total=%u\n",
				         headerSize, sizeof(Matrix4f*), sizeof(Matrix4f), numTextures, texSize);
				sReportedFrameCacheLayout = true;
			}
#endif
...
void BaseShape::updateAnim(Graphics& gfx, immut Matrix4f& mtx, f32* p3)
{
...
#if defined(TARGET_PC)
	static int sAnimReports = 0;
	pikmin_set_crash_context("updateAnim begin model=%s shape=%p matrices=%p joints=%d envelopes=%d animMtx=%d animCtx=%p animData=%p",
	                         mName ? mName : "<unnamed>", this, mAnimMatrices, mJointCount, mEnvelopeCount, mAnimMtxCount,
	                         mCurrentAnimation, mCurrentAnimation ? mCurrentAnimation->mData : nullptr);
	if (!mAnimMatrices || !mCurrentAnimation || (mJointCount > 0 && !mJointList) || mJointCount < 0 || mAnimMtxCount < mJointCount) {
		if (sAnimReports < 32) {
			OSReport("[pikmin::shape] invalid anim matrix buffer model=%s matrices=%p joints=%d animMtxCount=%d\n",
			         mName ? mName : "<unnamed>", mAnimMatrices, mJointCount, mAnimMtxCount);
			sAnimReports++;
		}
	...
	if (mCurrentAnimation->mData) {
		...
		for (int i = 0; i < mJointCount; i++) {
			AnimData* data = (mAnimOverrides && mAnimOverrides[i]) ? mAnimOverrides[i]->mData : nullptr;
			int parentIndex = mJointList[i].mParentIndex;

#if defined(TARGET_PC)
			if (parentIndex < -1 || parentIndex >= mJointCount) {
				if (sAnimReports < 32) {
					OSReport("[pikmin::shape] bad anim parent model=%s joint=%d parent=%d joints=%d; using root\n",
					         mName ? mName : "<unnamed>", i, parentIndex, mJointCount);
					sAnimReports++;
				}
				parentIndex = -1;
			}
			...
			immut Matrix4f* srtMtx = (parentIndex != -1) ? &mAnimMatrices[parentIndex] : &mtx;

			// Zero-frame placeholder animations deliberately do not need packed joint
			// indices; the original path simply uses the model's precomputed base pose.
			const bool missingAnimData = !data || i >= data->mJointCount
			                         || (data->mTotalFrameCount > 0 && (!data->mAnimInfo || !data->mAnimJointIndices));
			if (missingAnimData) {
#if defined(TARGET_PC)
				if (sAnimReports < 32) {
					OSReport("[pikmin::shape] animation joint mismatch model=%s joint=%d modelJoints=%d anim=%p animJoints=%d frames=%d info=%p map=%p; base pose fallback\n",
					         mName ? mName : "<unnamed>", i, mJointCount, data, data ? data->mJointCount : -1,
					         data ? data->mTotalFrameCount : -1, data ? data->mAnimInfo : nullptr, data ? data->mAnimJointIndices : nullptr);
					sAnimReports++;
				}
#endif
				SRT baseSrt;
				baseSrt.s = mJointList[i].mScale;
				baseSrt.r = mJointList[i].mRotation;
				baseSrt.t = mJointList[i].mTranslation;
				Matrix4f localMtx;
				mAnimMatrices[i].makeConcatSRT(srtMtx, localMtx, baseSrt);
				continue;
			}
...
#if defined(TARGET_PC)
			if (data->mTotalFrameCount > 0) {
				if (*frame < 0.0f) {
					if (sAnimReports < 32) {
						OSReport("[pikmin::shape] negative anim frame model=%s joint=%d frame=%f; clamping\n",
						         mName ? mName : "<unnamed>", i, *frame);
						sAnimReports++;
					}
					*frame = 0.0f;
				} else if (int(*frame) >= data->mTotalFrameCount) {
					if (sAnimReports < 32) {
						OSReport("[pikmin::shape] anim frame overflow model=%s joint=%d frame=%f frames=%d; clamping\n",
						         mName ? mName : "<unnamed>", i, *frame, data->mTotalFrameCount);
						sAnimReports++;
					}
					*frame = static_cast<f32>(data->mTotalFrameCount - 1);
				}
			}
#endif

			if (mFrameCacher && data->mAnimFlags & ANIMFLAG_UseCache && data->mTotalFrameCount > 0 && data->mAnimInfoList) {
				int frameNum = int(*frame);
				if (frameNum >= 0 && frameNum < data->mTotalFrameCount) {
			...
			if (data->mTotalFrameCount) {
				int animJoint = data->mAnimJointIndices[i];
#if defined(TARGET_PC)
				if (animJoint < 0 || (data->mActiveJointCount > 0 && animJoint >= data->mActiveJointCount)) {
					if (sAnimReports < 32) {
						OSReport("[pikmin::shape] bad anim joint map model=%s joint=%d animJoint=%d active=%d; base pose fallback\n",
						         mName ? mName : "<unnamed>", i, animJoint, data->mActiveJointCount);
						sAnimReports++;
					}
					SRT baseSrt;
					baseSrt.s = mJointList[i].mScale;
...
#endif
#if defined(TARGET_PC)
				pikmin_set_crash_context("updateAnim animSRT model=%s shape=%p joint=%d/%d parent=%d anim=%p animJoint=%d active=%d frame=%f/%d src=%p dst=%p",
				                         mName ? mName : "<unnamed>", this, i, mJointCount, parentIndex, data, animJoint,
				                         data->mActiveJointCount, *frame, data->mTotalFrameCount, srtMtx, &mAnimMatrices[i]);
#endif
				data->makeAnimSRT(animJoint, srtMtx, &mAnimMatrices[i], &data->mAnimInfo[i], *frame);
			} else {
#if defined(WIN32) || defined(TARGET_PC)
#if defined(TARGET_PC)
				pikmin_set_crash_context("updateAnim basePose model=%s shape=%p joint=%d/%d src=%p modelMtx=%p dst=%p",
				                         mName ? mName : "<unnamed>", this, i, mJointCount, &mtx, &mJointList[i].mAnimMatrix, &mAnimMatrices[i]);
#endif
...
#if defined(TARGET_PC)
		pikmin_set_crash_context("updateAnim weighted begin model=%s shape=%p joints=%d envelopes=%d matrices=%p",
		                         mName ? mName : "<unnamed>", this, mJointCount, mEnvelopeCount, mAnimMatrices);
#endif
		...
		for (int i = 0; i < mJointCount; i++) {
			...
			Matrix4f tmpMtx;
			int parentIndex = joint->mParentIndex;
#if defined(TARGET_PC)
			if (parentIndex < -1 || parentIndex >= mJointCount) {
				parentIndex = -1;
			}
#endif
			immut Matrix4f* jointMtx = (parentIndex == -1) ? &mtx : &mJointList[parentIndex].mAnimMatrix;
...
#if defined(TARGET_PC)
	pikmin_set_crash_context("updateAnim complete model=%s shape=%p joints=%d envelopes=%d",
	                         mName ? mName : "<unnamed>", this, mJointCount, mEnvelopeCount);
#endif
	gsys->mTimer->stop("updateAnim");
}
...
#define PIKI_SHAPE_REGISTER register
#else
#define PIKI_SHAPE_REGISTER
#endif
static inline void addMatrixWeights(PIKI_SHAPE_REGISTER f32* animMtx, PIKI_SHAPE_REGISTER f32* weightedMtx, PIKI_SHAPE_REGISTER f32 weights[2])
{
#ifdef __MWERKS__
	asm {
...
{
#if defined(TARGET_PC)
	static int sEnvelopeIndexReports = 0;
#endif

	for (int envIdx = 0; envIdx < mEnvelopeCount; envIdx++) {
#if defined(TARGET_PC)
		Envelope& envelope = mEnvelopeList[envIdx];
		if (envelope.mIndexCount < 0 || envelope.mIndexCount > mJointCount || (envelope.mIndexCount > 0 && (!envelope.mJointIndices || !envelope.mWeights))) {
			if (sEnvelopeIndexReports < 64) {
				OSReport("[pikmin::shape] invalid envelope model=%s env=%d/%d count=%d joints=%d indices=%p weights=%p; skipping\n",
				         mName ? mName : "<unnamed>", envIdx, mEnvelopeCount, envelope.mIndexCount, mJointCount, envelope.mJointIndices, envelope.mWeights);
				sEnvelopeIndexReports++;
			}
			continue;
		}
		pikmin_set_crash_context("weighted envelope model=%s shape=%p env=%d/%d count=%d joints=%d matrices=%p",
		                         mName ? mName : "<unnamed>", this, envIdx, mEnvelopeCount, envelope.mIndexCount, mJointCount, mAnimMatrices);
#endif
...
#if defined(TARGET_PC)
			pikmin_set_crash_context("weighted slot model=%s shape=%p env=%d/%d slot=%d/%d idx=%d joints=%d weight=%f anim=%p inv=%p dst=%p",
			                         mName ? mName : "<unnamed>", this, envIdx, mEnvelopeCount, i, mEnvelopeList[envIdx].mIndexCount,
			                         idx, mJointCount, weight,
			                         (idx >= 0 && idx < mJointCount) ? &mAnimMatrices[idx] : nullptr,
			                         (idx >= 0 && idx < mJointCount) ? &mJointList[idx].mInverseAnimMatrix : nullptr,
			                         &mAnimMatrices[mJointCount + envIdx]);
#endif

#if defined(TARGET_PC)
			// A bad envelope index turns both matrix inputs below into invalid
			// pointers and crashes inside C_MTXConcat. Diagnose the common
			// byte-swapped-u16 case, repair it only when it resolves to a real
			// joint, and otherwise skip the corrupt weight instead of indexing
			// outside the model.
			if (idx < 0 || idx >= mJointCount) {
				u16 raw     = static_cast<u16>(idx);
				int swapped = static_cast<int>(static_cast<u16>((raw >> 8) | (raw << 8)));
				if (swapped >= 0 && swapped < mJointCount) {
					if (sEnvelopeIndexReports < 32) {
						OSReport("[pikmin::shape] envelope joint endian repair model=%s env=%d slot=%d idx=%d -> %d joints=%d\n",
						         mName ? mName : "<unnamed>", envIdx, i, idx, swapped, mJointCount);
					}
					idx = swapped;
				} else {
					if (sEnvelopeIndexReports < 32) {
						OSReport("[pikmin::shape] bad envelope joint model=%s env=%d slot=%d idx=%d joints=%d; skipping\n",
						         mName ? mName : "<unnamed>", envIdx, i, idx, mJointCount);
					}
					sEnvelopeIndexReports++;
					continue;
				}
...
#if defined(WIN32) || defined(TARGET_PC)
			// The PowerPC helper below is paired-single assembly and compiles to
			// an empty function outside MWERKS. Native PC builds must use the
			// generic accumulation path or skinned envelope matrices stay zero.
			for (int count = 0; count < 12; count++) {
...
#if defined(TARGET_PC)
		// The native Matrix4f representation has a fourth row while the GameCube
		// envelope path is affine 3x4. Keep that row deterministic for later PC math.
		Matrix4f& envMtx = mAnimMatrices[mJointCount + envIdx];
		envMtx.mMtx[3][0] = 0.0f;
		envMtx.mMtx[3][1] = 0.0f;
		envMtx.mMtx[3][2] = 0.0f;
		envMtx.mMtx[3][3] = 1.0f;
#endif
	}
}
