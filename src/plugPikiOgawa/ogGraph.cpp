...
{
	u32 graphColor = (u32(og_piki_lines_color[pikminColor][0]) << 24)
	               | (u32(og_piki_lines_color[pikminColor][1]) << 16)
	               | (u32(og_piki_lines_color[pikminColor][2]) << 8)
	               | u32(og_piki_lines_color[pikminColor][3]);
	...
	if (hours >= 2) {
		...
		GXSETARRAY(GX_VA_POS, vertexData, hours * sizeof(u32), sizeof(u32), true);
		GXSETARRAY(GX_VA_CLR0, og_piki_lines_color, sizeof(og_piki_lines_color), sizeof(og_piki_lines_color[0]), true);
