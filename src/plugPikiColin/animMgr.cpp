...
void AnimMgr::loadAnims(immut char* animPath, immut char* bundlePath)
{
	...
	if (!mSkipLoading) {
		...
		if (missingAnimCount != 0) {
			...
			// The original matching code relies on the Metrowerks stack layout:
			// finalAnimPath sits immediately before finalBundlePath, so indexing
			// finalAnimPath by strlen(finalBundlePath) + 253 reaches the final
			// three bytes of finalBundlePath and changes "mod" to "anm".
			// Clang is free to lay these arrays out differently, turning that
			// expression into an out-of-bounds stack write. Build the path
			// explicitly on native hosts instead.
			const char* sourcePath = bundlePath ? bundlePath : mModel->mName;
			const size_t sourceLen = sourcePath ? strlen(sourcePath) : 0;
			if (!sourcePath || sourceLen == 0 || sourceLen >= sizeof(finalBundlePath)) {
				OSPanic(__FILE__, __LINE__, "AnimMgr bundle path invalid/too long");
			}
			memcpy(finalBundlePath, sourcePath, sourceLen + 1);

			if (!bundlePath) {
				char* extension = strrchr(finalBundlePath, '.');
				if (!extension || strlen(extension) != 4) {
					OSPanic(__FILE__, __LINE__, "AnimMgr expected model path ending in .mod: %s", finalBundlePath);
				}
				memcpy(extension + 1, "anm", 4);
			}
			OSReport("[pikmin::anim] bundle source=%s resolved=%s missing=%d\n", sourcePath, finalBundlePath, missingAnimCount);
#else
