...
static s32 DoErase(s32 chan, s32 fileNo)
{
	...
	// Aurora's default GCI-folder backend can expose directory slots differently
	// from the original SDK's fast-index path. Resolve the slot to its canonical
	// filename first, then delete by name. This is especially important for
	// Pikmin's stale temporary save ("Pikmin..." / "~Pikmin...") cleanup.
	CARDStat stat {};
	int statResult = CARDGetStatus(chan, fileNo, &stat);
	if (statResult < 0) {
		OSReport("[pikmin::card] erase failed to resolve index=%d result=%d\n", fileNo, statResult);
		return statResult;
	}
	int resDel = CARDDelete(chan, stat.fileName);
	OSReport("[pikmin::card] erase index=%d name=%s result=%d\n", fileNo, stat.fileName, resDel);
#else
...
static s32 DoSave(s32 chan, CARDStat* fileState, void* fileData)
{
	...
	if (fileState->commentAddr <= fileState->length - CARD_COMMENT_SIZE) {
		memmove(entry->mFileCommentData, (void*)((uptr)fileData + fileState->commentAddr), CARD_COMMENT_SIZE);
	}

	// handle icon/banner data, if present
	entry->mAnimTotalFrames = 0;
	if (fileState->bannerFormat || fileState->iconFormat) {
		memmove(entry, (void*)((uptr)fileData + fileState->iconAddr), fileState->offsetData - fileState->iconAddr);
 ...
 * @return CARD result code from last operation hit - 0 or 1 if no error, negative if error.
 */
static s32 CardUtilCommand(s32 chan, s32 command, intptr_t file, void* workBuffer, u32 offset, u32 length)
{
#if defined(TARGET_PC)
	// The original SDK utility deliberately serialized CARD operations on a
	// dedicated GameCube thread. Aurora's native card image backend is already
	// synchronous, and running it from our std::thread shim races section/heap
	// transitions and has been the last live game worker on the host. Execute the
	// exact same Do* helpers synchronously instead. Callers already wait through
	// CardUtilIdleWhileBusy(), which becomes an immediate no-op after this call.
	OSLockMutex(&CardControl.mMutex);
	if (CardControl.mChannel != -1) {
		s32 busyResult = CardControl.mResultCode;
	...
	CardControl.mFileNo     = reinterpret_cast<void*>(file);
	...
	s32 result = CARD_RESULT_FATAL_ERROR;
	switch (command) {
	case CARDCMD_Mount:
		result = DoMount(chan, workBuffer);
		break;
	case CARDCMD_Unmount:
		result = DoUnmount(chan);
		break;
	case CARDCMD_Format:
		result = DoFormat(chan);
		break;
	case CARDCMD_List:
		result = DoList(chan, static_cast<CardUtilDirent*>(workBuffer));
		break;
	case CARDCMD_Erase:
		result = DoErase(chan, static_cast<s32>(file));
		break;
	case CARDCMD_Open:
		result = DoOpen(chan, static_cast<s32>(file), workBuffer);
		break;
	case CARDCMD_Save:
		result = DoSave(chan, reinterpret_cast<CARDStat*>(file), workBuffer);
		break;
	case CARDCMD_Write:
		result = DoWrite(chan, static_cast<s32>(file), workBuffer, length, offset);
		break;
	default:
		OSReport("[pikmin::card] unknown synchronous CardUtil command=%d\n", command);
		break;
	}
	...
	CardControl.mResultCode = result;
	CardControl.mChannel    = -1;
	OSUnlockMutex(&CardControl.mMutex);
	OSReport("[pikmin::card] sync command=%d channel=%d result=%d\n", command, chan, result);
	return CARD_RESULT_READY;
#else
	OSLockMutex(&CardControl.mMutex);
	int res;
...
void CardUtilSave(s32 chan, CARDStat* fileState, void* data)
{
	...
	CardUtilCommand(chan, CARDCMD_Save, reinterpret_cast<intptr_t>(fileState), data, 0, 0);
}
...
void CardUtilInit(void* stack, u32 stackSize, s32 prio)
{
	...
	CardControl.mChannel    = -1;
	CardControl.mResultCode = CARD_RESULT_READY;
#if defined(TARGET_PC)
	(void)stack;
	(void)stackSize;
	(void)prio;
	OSReport("[pikmin::card] host CardUtil worker disabled; using synchronous Aurora CARD operations\n");
#else
