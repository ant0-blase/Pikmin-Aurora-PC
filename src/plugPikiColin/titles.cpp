...
public:
	...
	TitleSetupSection()
	{
		...
		OSReport("[pikmin::titles] TitleSetupSection ctor begin this=%p sizeof=%zu\n", this, sizeof(*this));
#endif
		setName("Titles section");
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] TitleSetupSection name initialized\n");
#endif

#if defined(BUGFIX) || defined(TARGET_PC)
		// This section uses `SeSystem` without constructing it in the original retail path.
		if (!seSystem) {
			seSystem = new SeSystem();
		}
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] SeSystem ready=%p\n", seSystem);
#endif
#endif
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] movie interface=%p\n", gameflow.mGameInterface);
#endif
		mController = new Controller(1);
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] title controller=%p\n", mController);
#endif
		mState = STATE_Active;

#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing ogScrTitleMgr\n");
#endif
		titleWindow = new zen::ogScrTitleMgr;
		titleWindowOn = false;
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] ogScrTitleMgr=%p\n", titleWindow);
#endif

		mDayMgr = new DayMgr(nullptr, mController);
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] DayMgr=%p\n", mDayMgr);
#endif

		// play camera pan-in to title screen (which will also trigger PRESS START to show up)
		gameflow.mDemoFlags = 0;
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing opening CinematicPlayer\n");
#endif
		mPlayer = new CinematicPlayer("cinemas/opening.cin");
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] CinematicPlayer=%p\n", mPlayer);
#endif
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] opening.ini stream=%p\n", data);
#endif
		if (data) {
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] opening.ini parse complete\n");
#endif
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing debug menu\n");
#endif
		mDebugMenu = new Menu(mController, gsys->mConsFont);
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] debug menu=%p\n", mDebugMenu);
#endif
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing AnimFrameCacher\n");
#endif
		gameflow.mFrameCacher = new AnimFrameCacher(8000);
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] AnimFrameCacher=%p\n", gameflow.mFrameCacher);
#endif
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing DrawHiScore\n");
#endif
		totalWindow = new zen::DrawHiScore;
		totalWindowOn = false;
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] DrawHiScore=%p\n", totalWindow);
#endif

		// set up start menu
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing PRESS START manager\n");
#endif
		startWindow = new zen::ogScrStartMgr;
		startWindowOn = false;
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] PRESS START manager=%p\n", startWindow);
#endif

		gsys->setFade(1.0f);
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] TitleSetupSection ctor complete this=%p\n", this);
#endif
	}
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] TitlesSection::init begin this=%p vptr=%p heap=%d\n", this, *reinterpret_cast<void**>(this), gsys->mActiveHeapIdx);
#endif
	Node::init("<TitlesSection>");
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] Node::init complete\n");
#endif
	Jac_BackDVDBuffer();
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] Jac_BackDVDBuffer complete\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] basic title state configured; probing memory card\n");
#endif
#if defined(VERSION_PIKIDEMO)
	// no save games/memory card for demo
#else
	s32 cardState = gameflow.mMemoryCard.getMemoryCardState(true);
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] memory card state=%d saveIndex=%d tempIndex=%d channel=%d\n", cardState,
	         gameflow.mMemoryCard.mSaveFileIndex, gameflow.mMemoryCard.mTempFileIndex, gameflow.mMemoryCard.mCardChannel);
#endif
	if (cardState == CARD_RESULT_READY && gameflow.mMemoryCard.mSaveFileIndex >= 0) {
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] loading saved options\n");
#endif
		gameflow.mMemoryCard.loadOptions();
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] saved options loaded\n");
#endif
	}
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] sound mode synchronized\n");
#endif
	gsys->startLoading(&gameflow.mGameLoadIdler, true, gameflow.mIsNintendoLoadLogo ? 0 : 60);
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] loading state started\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] language before=%d after=%d\n", beforeLang, gameflow.mLanguageIndex);
#endif
	if (gameflow.mLanguageIndex != beforeLang) {
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] reloading language bundle\n");
#endif
		preloadLanguage();
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] language reload complete\n");
#endif
	}
...
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] constructing TitleSetupSection\n");
#endif
		TitleSetupSection* setup = new TitleSetupSection;
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] TitleSetupSection allocated=%p; attaching to TitlesSection\n", setup);
#endif
		add(setup);
#if defined(TARGET_PC)
		OSReport("[pikmin::titles] TitleSetupSection attached\n");
#endif
		break;
	}
	}
...
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] ending loading state\n");
#endif
	PRINT("ending loading!\n");
	gsys->endLoading();
	PRINT("done!\n");
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] loading state ended; scene setup\n");
#endif

	Jac_SceneSetup(SCENE_Title, 0);
#if defined(TARGET_PC)
	OSReport("[pikmin::titles] TitlesSection::init complete\n");
#endif
}
