...
void GamePrefs::read(RandomAccessStream& input)
{
	...
	// The original reload path deliberately re-applies every option through its
	// setter.  On GameCube that is harmless, but on the native port it also
	// resets the motor subsystem (four PADControlMotor calls) while SDL/Aurora is
	// live.  Saved options are plain data, so restore them without those GC-only
	// side effects and validate values read from persistent host storage.
	mFlags &= (GAMEPREF_Vibe | GAMEPREF_Stereo | GAMEPREF_Child);
	if (mBgmVol > 10) mBgmVol = 8;
	if (mSfxVol > 10) mSfxVol = 8;
	mChalCourseOpenFlags &= 0x1f;
	mChangesPending = false;

	Jac_SetBGMVolume(mBgmVol);
	Jac_SetSEVolume(mSfxVol);
	Jac_OutputMode((mFlags & GAMEPREF_Stereo) ? 1 : 0);
	OSSetSoundMode((mFlags & GAMEPREF_Stereo) != 0);

	OSReport("[pikmin::prefs] host restore flags=%08x bgm=%u sfx=%u challenge=%02x totalPikis=%d\n",
	         mFlags, mBgmVol, mSfxVol, mChalCourseOpenFlags, mHiscores.mTotalPikis);
#else
