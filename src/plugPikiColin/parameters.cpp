	...
	while (true) {
...
#if defined(TARGET_PC)
		if (hostEntryCount++ > 4096) {
			OSPanic(__FILE__, __LINE__, "Parameter stream exceeded sane entry count");
		}
#endif
		if (entryHeader == FILE_TERMINATOR) {
#if defined(TARGET_PC)
			OSReport("[pikmin::param] terminator path=%s entries=%d pos=%d\n",
			         input.mPath ? input.mPath : "<unknown>", hostEntryCount - 1, input.getPosition());
#endif
			// end of file reached
			break;
		}
		...
		FOREACH_NODE(BaseParm, mFirstParm, node)
		{
			if (id == node->mID.Num()) {
				inList = true;
#if defined(TARGET_PC)
				if (hostEntryCount <= 16) {
					OSReport("[pikmin::param] matched id=%08x size=%d pos=%d\n", id, entrySize, input.getPosition());
				}
#endif
				node->read(input);
				break;
