...
// Raw memory-card artwork. Modern C++ rejects list-initializing signed char
// with byte values 0x80..0xFF, so use an unsigned byte on the native host
// while preserving the original char type for matching GameCube builds.
#if defined(TARGET_PC)
typedef u8 MemoryCardRawByte;
#else
typedef char MemoryCardRawByte;
#endif
u8 cardData[CARD_DATA_SIZE];
static u8 CardStack[0x2000];
static immut char basecardname[]                            = "Pikmin dataFile";
static immut MemoryCardRawByte p_icon_palette[0x200] ATTRIBUTE_ALIGN(32) = {
...
static immut MemoryCardRawByte p_icon_data[0x400] = {
...
static immut MemoryCardRawByte o_icon_palette[0x200] = {
	0x80, 0x00, 0xFF, 0xFF, 0xFE, 0x60, 0xDD, 0xF0, 0xC1, 0x29, 0xBD, 0x4B, 0x94, 0x01, 0xB5, 0x2A, 0x80, 0x00, 0xBD, 0x09, 0xC5, 0xAD,
	0xB1, 0x6B, 0xF6, 0x54, 0x84, 0x00, 0xC9, 0x4C, 0x80, 0x00, 0xDD, 0x6F, 0x98, 0xC6, 0xC2, 0x10, 0xF0, 0x8C, 0xA4, 0xA6, 0xB0, 0x25,
	0xAD, 0x09, 0xD4, 0x8B, 0xF9, 0x73, 0xCA, 0x31, 0xA8, 0x25, 0xFE, 0x79, 0xBD, 0xCF, 0xCE, 0x52, 0xC6, 0x31, 0xB9, 0x6D, 0xFB, 0x1C,
	0xB4, 0x6A, 0xC9, 0x90, 0x98, 0x25, 0x88, 0x02, 0x84, 0x01, 0x80, 0x00, 0xFB, 0xBE, 0xB9, 0xCE, 0xB5, 0x8D, 0xB5, 0x8D, 0x80, 0x00,
...
static immut MemoryCardRawByte o_icon_data[0x400] = {
...
static immut MemoryCardRawByte p_card_v_palette[0x200] = {
...
static immut MemoryCardRawByte p_card_v_data[0xC00] = {
...
static immut MemoryCardRawByte* icon_datas[] = { p_icon_data, nullptr, nullptr, p_icon_data, nullptr, nullptr, nullptr, o_icon_data };
static immut MemoryCardRawByte* icon_pals[]  = { p_icon_palette };
...
bool MemoryCard::attemptFormatCard(int channel)
{
	if (CARDProbe(channel)) {
#if defined(TARGET_PC)
		// Aurora implements the CARD API synchronously on the host.  Its Async
		// wrappers invoke callbacks immediately and currently assume they are
		// non-null, while the original Pikmin code intentionally passed null.
		// Use the synchronous entry point instead of calling through a null
		// callback.
		s32 mountResult = CARDMount(channel, &CardWorkArea, nullptr);
		OSReport("[pikmin::card] format mount channel=%d result=%d\n", channel, mountResult);
		if (mountResult != CARD_RESULT_READY && mountResult != CARD_RESULT_BROKEN && mountResult != CARD_RESULT_ENCODING) {
		...
		{
#if defined(TARGET_PC)
			s32 formatResult = CARDFormat(channel);
			OSReport("[pikmin::card] format channel=%d result=%d\n", channel, formatResult);
			if (formatResult < 0) {
				CARDUnmount(channel);
				mErrorCode = CARD_RESULT_BROKEN;
				return false;
...
bool MemoryCard::getCardStatus(int channel)
{
	if (CARDProbe(channel)) {
#if defined(TARGET_PC)
		// The native Aurora CARD backend is synchronous.  Do not use the
		// Async wrapper here: the GameCube code passes null callbacks, whereas
		// Aurora's wrapper invokes them immediately.
		s32 res = CARDMount(channel, &CardWorkArea, nullptr);
		OSReport("[pikmin::card] mount channel=%d result=%d\n", channel, res);
		if (res != CARD_RESULT_READY && res != CARD_RESULT_BROKEN && res != CARD_RESULT_ENCODING) {
			mErrorCode = CARD_RESULT_WRONGDEVICE;
			return false;
		}
#else
		...
		case CARD_RESULT_BROKEN:
		{
			...
			OSReport("[pikmin::card] check channel=%d result=%d\n", channel, res);
#else
...
s32 MemoryCard::getMemoryCardState(bool flag)
{
	...
	for (int channel = 0; channel < 1; channel++) {
		...
		if (flag && mCardChannel >= 0 && mSaveFileIndex >= 0) {
			// Only load a file that actually exists. The original condition also
			// accepted -1 (valid card, no Pikmin save), causing CARDGetStatus/Open(-1)
			// on the native backend and making a fresh card look corrupt.
			loadCurrentFile();
			mErrorCode = CARD_RESULT_READY;
...
#if defined(TARGET_PC)
	if (mSaveFileIndex < 0) {
		OSReport("[pikmin::card] refusing to load invalid save index=%d\n", mSaveFileIndex);
		return;
	}
#endif
	CardUtilMount(0, &CardWorkArea);
