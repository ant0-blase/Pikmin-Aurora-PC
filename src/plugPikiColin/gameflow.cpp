...
void GameFlow::hardReset(BaseApp* baseApp)
{
	...
	OSReport("[pikmin::boot] GameFlow::hardReset begin app=%p\n", baseApp);
#endif
	app  = baseApp;
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow language paths configured\n");
#endif
	mParameters = new Parms();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow parms object=%p\n", mParameters);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow initializing MemoryCard thread\n");
#endif
	mMemoryCard.init();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow MemoryCard init done\n");
#endif

	// set up load heap
	PRINT("load heap\n");
	u8* loadHeapBuffer = new u8[loadHeapSize];
	gsys->mHeaps[SYSHEAP_Load].init("load", AYU_STACK_GROW_UP, loadHeapBuffer, loadHeapSize);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow load heap=%p size=%d\n", loadHeapBuffer, loadHeapSize);
#endif

	// prepare nintendo logo to appear
	mLastLoadedBannerTex  = nullptr;
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow loading intro/nintendo.bti\n");
#endif
	mLevelBannerTex       = setLoadBanner("intro/nintendo.bti");
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow nintendo banner=%p\n", mLevelBannerTex);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow loading thread started\n");
#endif
	...
	PRINT("load params\n");
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow loading parms/gamePrms.bin\n");
#endif
	load("parms/", "gamePrms.bin", 1);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow gamePrms loaded\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow MoviePlayer=%p\n", mMoviePlayer);
#endif

	// set up controller rumble manager
	rumbleMgr = new RumbleMgr(true, false, false, false);
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow RumbleMgr=%p\n", rumbleMgr);
#endif
	rumbleMgr->reset();
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow GamePrefs initialized\n");
#endif

	mFlowManager = new GameGenFlow("GameGenFlow");
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow flow manager=%p\n", mFlowManager);
#endif

	// load effects
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow loading effects bundles\n");
#endif
...
{
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow::softReset begin next=%d current=%d\n", mNextGameSectionID, mCurrGameSectionID);
#endif
...
#if defined(TARGET_PC)
		OSReport("[pikmin::boot] GameFlow NinLogoSection=%p\n", mGameSection);
#endif
			break;
		}
		case SECTION_Titles:
...
#if defined(TARGET_PC)
			OSReport("[pikmin::titles] TitlesSection allocated=%p vptr=%p sizeof=%zu\n", mGameSection,
			         mGameSection ? *reinterpret_cast<void**>(mGameSection) : nullptr, sizeof(TitlesSection));
#endif
			break;
		}
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow initializing flow manager\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow initializing game section=%p id=%d\n", mGameSection, mNextGameSectionID);
#endif
	mGameSection->init();
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow game section init complete\n");
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::boot] GameFlow::softReset complete current=%d\n", mCurrGameSectionID);
#endif
}
