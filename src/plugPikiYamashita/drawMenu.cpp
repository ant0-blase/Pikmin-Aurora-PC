    ...
    : DrawScreen(bloFileName, nullptr, useAlphaMgr, useTexAnimMgr)
{
	...
	OSReport("[pikmin::drawmenu] body begin path=%s this=%p\n", bloFileName ? bloFileName : "<null>", this);
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::drawmenu] path=%s pall=%p\n", bloFileName, mParentPane);
#endif
	if (!mParentPane) {
		OSPanic(__FILE__, __LINE__, "DrawMenu %s missing required pane 'pall'", bloFileName);
	}
	mParentPane->setOffset(mParentPane->getWidth() >> 1, mParentPane->getHeight() >> 1);
...
#if defined(TARGET_PC)
	OSReport("[pikmin::drawmenu] path=%s se_c=%p\n", bloFileName, paneAlso);
#endif
	if (!paneAlso) {
		OSPanic(__FILE__, __LINE__, "DrawMenu %s missing required pane 'se_c'", bloFileName);
	}
...
#if defined(TARGET_PC)
	OSReport("[pikmin::drawmenu] path=%s selectCount=%d\n", bloFileName, mSelectCount);
#endif
	...
	mSpecCount = i;
#if defined(TARGET_PC)
	OSReport("[pikmin::drawmenu] path=%s specCount=%d\n", bloFileName, mSpecCount);
#endif
	...
	updateMenuPanes();
#if defined(TARGET_PC)
	OSReport("[pikmin::drawmenu] ctor complete path=%s\n", bloFileName);
#endif
}
