...
void P2DTextBox::loadResource()
{
	...
	OSReport("[pikmin::p2d-res] textbox begin pane=%p tag=%08x font=%s\n",
	         this, mTagName, mFontString ? mFontString : "<null>");
	if (!mFontString || !*mFontString) {
		OSReport("[pikmin::p2d-res] textbox has no font; leaving font null pane=%p\n", this);
		mFont = nullptr;
		return;
	}
#endif
...
#if defined(TARGET_PC)
	OSReport("[pikmin::p2d-res] textbox ok pane=%p fontObj=%p name=%s\n", this, mFont, mFontString);
#endif
}
