...
Font* P2DFont::loadFont(immut char* name, int& rows, int& columns)
{
	...
	// The original P2D path caches a synthetic Font object in StdSystem's
	// GfxobjInfo list, even though all of these .bfn names use bigFont.bti.
	// That list spans multiple custom stack heaps and is especially fragile
	// across native 64-bit section resets.  Reuse the canonical system font
	// created at boot instead; it lives for the entire process and already has
	// the exact 21x42 glyph metrics expected by P2D.
	extern Font* PikminHostGetBigFont();
	rows    = 21;
	columns = 42;
	Font* hostFont = PikminHostGetBigFont();
	OSReport("[pikmin::p2d-font] request=%s systemFont=%p texture=%p\n",
	         name ? name : "<null>", hostFont, hostFont ? hostFont->mTexture : nullptr);
	if (!hostFont || !hostFont->mTexture) {
		OSPanic(__FILE__, __LINE__, "P2D requested font before system bigFont was ready");
	}
	return hostFont;
#else
	Font* font       = nullptr;
