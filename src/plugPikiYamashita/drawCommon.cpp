...
zen::DrawScreen::DrawScreen(immut char* bloFileName, P2DGrafContext* grafContext, bool useAlphaMgr, bool useTexAnimMgr)
{
	...
	OSReport("[pikmin::drawscreen] set begin path=%s this=%p\n", bloFileName ? bloFileName : "<null>", this);
#endif
	mScreen.set(bloFileName, useAlphaMgr, useTexAnimMgr, true);
#if defined(TARGET_PC)
	OSReport("[pikmin::drawscreen] set complete path=%s\n", bloFileName ? bloFileName : "<null>");
#endif
	if (grafContext) {
...
#if defined(TARGET_PC)
	OSReport("[pikmin::drawscreen] ctor complete path=%s graf=%p\n", bloFileName ? bloFileName : "<null>", mGrafContext);
#endif
}
