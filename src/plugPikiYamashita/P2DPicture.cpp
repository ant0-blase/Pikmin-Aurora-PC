...
void P2DPicture::loadResource()
{
	...
	OSReport("[pikmin::p2d-res] picture begin pane=%p tag=%08x tex=%s\n",
	         this, mTagName, mTexName ? mTexName : "<null>");
#endif
	if (!mTexName) {
		PRINT("No texName.\n");
		return;
	...
	if (tex) {
		...
		mTextureCount++;
#if defined(TARGET_PC)
		OSReport("[pikmin::p2d-res] picture ok pane=%p texObj=%p name=%s size=%dx%d\n",
		         this, tex, mTexName, tex->mWidth, tex->mHeight);
#endif
	} else {
