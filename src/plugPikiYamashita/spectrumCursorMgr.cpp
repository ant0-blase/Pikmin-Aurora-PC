...
{
	char buf[8] = {};
	auto writeTag = [](char* dst, u32 tag) {
		dst[0] = char((tag >> 24) & 0xFF);
		dst[1] = char((tag >> 16) & 0xFF);
		dst[2] = char((tag >> 8) & 0xFF);
		dst[3] = char(tag & 0xFF);
		dst[4] = '\0';
	};
	mPaneCount = 0;
	writeTag(buf, makeTag(p3, mPaneCount));
...
u32 zen::SpectrumCursorMgr::makeTag(u32 tag, int idx)
{
	...
	buf[3] = char(tag & 0xFF);
	return (u32(u8(buf[0])) << 24) | (u32(u8(buf[1])) << 16) | (u32(u8(buf[2])) << 8) | u32(u8(buf[3]));
}
