...
bool GeneratorCache::hasUfoParts(u32 stageID, u32 ufoPartIdx)
{
	Cache* cache = findCache(mAliveCacheList, stageID);
	if (cache) {
		void* heap = (void*)(((uptr)mCacheHeap + cache->mCacheHeapOffset + cache->mGenCacheSize) + cache->mCreatureCacheSize);
...
void GeneratorCache::load(u32 stageID)
{
	...
	if (cache) {
		PRINT("cache = %x\n", cache);
		void* heap = (void*)(((uptr)mCacheHeap + cache->mCacheHeapOffset + cache->mGenCacheSize));
...
void GeneratorCache::saveGenerator(Generator* gen)
{
	if (gen->mDayLimit == -1 || gen->mDayLimit > gameflow.mWorldClock.mCurrentDay) {
		...
		void* heap = (void*)(((uptr)mCacheHeap + mUsedSize));
		RamStream stream(heap, mFreeSize);
		#if defined(TARGET_PC)
		PRINT("ramStream :%p - %p (%.2fK free)\n", heap, static_cast<void*>(static_cast<u8*>(heap) + mFreeSize), mFreeSize / 1024.0f);
#else
...
void GeneratorCache::loadUfoParts(GeneratorCache::Cache* cache)
{
	void* heap = (void*)(((uptr)mCacheHeap + cache->mCacheHeapOffset + cache->mGenCacheSize) + cache->mCreatureCacheSize);
	#if defined(TARGET_PC)
	PRINT("load from %p : %d ufo parts\n", heap, cache->mUfoPartsCount);
#else
