...
static void writeID(RandomAccessStream& output, u32 id)
{
	...
	// Generator files on disc were written by the original big-endian
	// implementation using id[3], id[2], id[1], id[0]. For a canonical
	// four-character ID (e.g. 'v0.1' == 0x76302E31), the on-disc bytes are
	// therefore low-byte first. Preserve the existing native RAM-save format
	// while ramMode is active so current Aurora saves remain compatible.
	if (!Generator::ramMode) {
		output.writeByte(static_cast<u8>(id));
		output.writeByte(static_cast<u8>(id >> 8));
		output.writeByte(static_cast<u8>(id >> 16));
		output.writeByte(static_cast<u8>(id >> 24));
		return;
	}
#endif
...
#if defined(TARGET_PC)
	if (!Generator::ramMode) {
		const u32 b0 = static_cast<u8>(input.readByte());
		const u32 b1 = static_cast<u8>(input.readByte());
		const u32 b2 = static_cast<u8>(input.readByte());
		const u32 b3 = static_cast<u8>(input.readByte());
...
static void readGeneratorID32(RandomAccessStream& input, ID32& id)
{
#if defined(TARGET_PC)
	if (!Generator::ramMode) {
		id.mId = readID(input);
		id.sprint(id.mStringID);
		return;
	}
#endif
	id.read(input);
...
static void writeGeneratorID32(RandomAccessStream& output, const ID32& id)
{
#if defined(TARGET_PC)
	if (!Generator::ramMode) {
		writeID(output, id.mId);
		return;
	}
#endif
	id.write(output);
...
void Generator::read(RandomAccessStream& input)
{
	...
	readGeneratorID32(input, mGeneratorName);
	if (!ramMode) {
		readGeneratorID32(input, mGeneratorVersion);
		_70 = readID(input);
	}
...
void Generator::write(RandomAccessStream& output)
{
	...
	writeGeneratorID32(output, mGeneratorName);

	if (!ramMode) {
		writeGeneratorID32(output, mGeneratorVersion);
		writeID(output, _70);
	}
...
void GeneratorMgr::read(RandomAccessStream& input, bool p2)
{
	...
	readGeneratorID32(input, mGeneratorVersionId);
	...
	mGenListHead = nullptr;
#if defined(TARGET_PC)
	OSReport("[pikmin::gen] manager=%p path=%s version=%08x count=%d pos=%d pending=%d ramMode=%d\n",
	         this, input.mPath ? input.mPath : "<unknown>", mGeneratorVersionId.mId, mGenCount,
	         input.getPosition(), input.getPending(), Generator::ramMode);
#endif
#if defined(TARGET_PC)
	if (!Generator::ramMode && (mGenCount < 0 || mGenCount > 4096)) {
		OSPanic(__FILE__, __LINE__, "Invalid generator count %d (version=%08x, pos=%d)", mGenCount, mGeneratorVersionId.mId,
		        input.getPosition());
	}
#endif
...
#if defined(TARGET_PC)
		if (i < 4 || i + 4 >= mGenCount) {
			OSReport("[pikmin::gen] entry=%d/%d begin pos=%d pending=%d\n", i + 1, mGenCount, input.getPosition(), input.getPending());
		}
#endif
		if (!mGenListHead) {
...
#if defined(TARGET_PC)
		if (i < 4 || i + 4 >= mGenCount) {
			OSReport("[pikmin::gen] entry=%d/%d complete pos=%d pending=%d\n", i + 1, mGenCount, input.getPosition(), input.getPending());
		}
#endif
	}
}
